...
@testing.parameterize(
    {'in_size': 10, 'out_size': 1},
    {'in_size': 20, 'out_size': 1},
)
class TestLSTM_Emphasis_Decoder(unittest.TestCase):

    def setUp(self):
        self.link = links.LSTM_Emphasis_Decoder(self.in_size, self.out_size)
        upward = self.link.upward.W.data
        upward[...] = numpy.random.uniform(-1, 1, upward.shape)
        lateral = self.link.lateral.W.data
        lateral[...] = numpy.random.uniform(-1, 1, lateral.shape)
        ...
        x_shape = (4, self.in_size)
        self.x = numpy.random.uniform(-1, 1, x_shape).astype(numpy.float32)
        self.src_hidden = numpy.random.uniform(-1, 1, x_shape).astype(numpy.float32)

    def check_forward(self, x_data, src_hidden_data):
        xp = self.link.xp
        x = chainer.Variable(x_data)
        src_hidden = chainer.Variable(src_hidden_data)
        h1 = self.link(x, src_hidden=src_hidden)
        c0 = chainer.Variable(xp.zeros((len(self.x), self.out_size),
                                       dtype=self.x.dtype))
        c1_expect, h1_expect = functions.lstm(c0, self.link.upward(x) + self.link.attention(src_hidden))
        gradient_check.assert_allclose(h1.data, h1_expect.data)
        gradient_check.assert_allclose(self.link.h.data, h1_expect.data)
        gradient_check.assert_allclose(self.link.c.data, c1_expect.data)

        h2 = self.link(x, src_hidden=src_hidden)
        c2_expect, h2_expect = \
            functions.lstm(c1_expect,
                           self.link.upward(x) + self.link.lateral(h1) + self.link.attention(src_hidden))
        gradient_check.assert_allclose(h2.data, h2_expect.data)

    def test_forward_cpu(self):
        self.check_forward(self.x, self.src_hidden)
